...
// DecryptCBC .. 解密
func DecryptCBC(keyBytes, ciphertextBytes []byte) (originalBytes []byte, err error) {
	...
	copy(iv, ciphertextBytes[:block.BlockSize()])

	// 初始化原文容器
	originalBytes = make([]byte, len(ciphertextBytes))
	...
	// 解密
	mode.CryptBlocks(originalBytes, ciphertextBytes)

	// 块数据解出
	padding := int(originalBytes[len(originalBytes)-1]) % block.BlockSize()
	originalBytes = originalBytes[block.BlockSize() : len(originalBytes)-padding]
